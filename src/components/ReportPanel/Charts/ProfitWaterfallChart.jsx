// src/components/ReportPanel/Charts/ProfitWaterfallChart.jsx
import React from 'react';
import BaseChart from '../../Charts/BaseChart';
import { formatCurrency } from '../../../utils/formatters';

export default function ProfitWaterfallChart({ calculatedData, periodIndex = 0 }) {
  ...
  const renderChartContent = (isRechartsLoaded) => {
    ...
    if (!calculatedData || calculatedData.length === 0 || !calculatedData[periodIndex]) {
      return <p className="text-center text-slate-500 py-4">Dados insuficientes para Profit Waterfall.</p>;
    }
    ...
    const waterfallData = [
      ...
      { name: 'L. Bruto', value: 0, cumulative: period.grossProfit || 0, type: 'milestone' },
      { name: 'Desp. Oper.', value: -(period.operatingExpenses || 0), cumulative: (period.grossProfit || 0) - (period.operatingExpenses || 0), type: 'negative' },
      { name: 'EBITDA', value: 0, cumulative: period.ebitda || 0, type: 'milestone' },
      ...
      { name: 'EBIT', value: 0, cumulative: period.ebit || 0, type: 'milestone' },
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-lg border border-slate-200 print:shadow-none print:border-slate-300 h-full flex flex-col">
        <h4 className="text-md font-semibold text-slate-800 mb-3 text-center print:text-sm">
          Cascata Lucratividade - Período {periodIndex + 1}
        </h4>
  ...
  return (
    <BaseChart libraryName="Recharts" chartTitle="Profit Waterfall">
      {renderChartContent}
    </BaseChart>
  );
}